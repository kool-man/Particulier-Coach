...
function ohSnap(text, color, icon) {
  ...
  // Generate the HTML
  var html = $('<div class="notif notif-' + color + '">' + icon_markup + text + '</div>');
  ...
  setTimeout(function () {
    ohSnapX($container.children('.notif').first());
  }, time);
}

function ohSnapX(element) {
  // Called without argument, the function removes all notifs
